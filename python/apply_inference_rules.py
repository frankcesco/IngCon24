def apply_inference_rules():
    ...
    updated_quartieri = list(prolog.query('quartiere(Id, Nome, Area, Pop2011, Densita)'))
    ...
    quartiere_arity = 5 if not updated_quartieri else len(updated_quartieri[0])
        ...
        for line in lines:
            ...
            if line.startswith('incidente('):
                ...
                for fact in updated_quartieri:
                    if fact['Id'] == original_id:
                        final_file.write(
                            f"quartiere('{fact['Id']}', '{fact['Nome']}', {fact['Area']}, {fact['Pop2011']}, {fact['Densita']}).\n")
                        break
            else:
